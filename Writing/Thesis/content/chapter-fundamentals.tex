...
\subsection{Loss}
While kendall can serve as an indication how well t works (causal connection exists in the thing trying to proove / algorithm finds it) the disadvantage is that just from the correlation it is not clear what the user gains from following the advice represented by the ranking, i.e. how much worse is it when trying the best three and chossing the best than actually showing the best
