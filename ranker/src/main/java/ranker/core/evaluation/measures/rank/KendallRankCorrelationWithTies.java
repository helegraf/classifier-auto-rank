...
public class KendallRankCorrelationWithTies extends RankerEvaluationMeasure {
	...
	public double evaluate(List<String> predictedRanking, List<String> perfectRanking, List<Double> estimates,
		...
		// Put those with the same actual performance value on the same rank because
		// their order is arbitrary?
		// But only for the reference-ranking because for the other one we dont actually
		// know
		for (int i = 0; i < performanceMeasures.size() - 1; i++) {
			if (performanceMeasures.get(i).equals(performanceMeasures.get(i + 1))
					|| (performanceMeasures.get(i).isNaN() && performanceMeasures.get(i + 1).isNaN())) {
